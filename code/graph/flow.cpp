template <const int MAXV, class flowUnit, class costUnit, const costUnit COST_INF, 
		const flowUnit FLOW_EPS = 0, const costUnit COST_EPS = 0, const int SCALE = 8>
struct FLOW {
	struct Edge {
		int to;
		flowUnit cap, resCap;
		costUnit cost;
		int rev;
		Edge(int to, flowUnit cap, costUnit cost, int rev) : 
			to(to), cap(cap), resCap(cap), cost(cost), rev(rev) {}
	};
	int cnt[MAXV * 2], h[MAXV], stk[MAXV], top;
	flowUnit maxFlow, ex[MAXV];
	costUnit phi[MAXV], bnd, minCost, negCost;
	...
	void addEdge(int v, int w, flowUnit flow, costUnit cost) {
		...
		adj[v].emplace_back(w, flow, cost, (long long)(adj[w].size()));
		adj[w].emplace_back(v, 0, -cost, (long long)(adj[v].size()) - 1);
	}
	void init(int V) {
		negCost = 0;
		for (int i = 0; i < V; i++) adj[i].clear();
	}
	flowUnit Flow(int V, int s, int t) {
			...
			for (int hi = 0; hi >= 0; ) {
				...
				while (ex[v] > FLOW_EPS) {
					if (cur[v] == adj[v].end()) {
						...
						if (--cnt[hi] == 0 && hi < V)
							for (int i = 0; i < V; i++) 
								if (hi < h[i] && h[i] < V) {cnt[h[i]]--; h[i] = V + 1;}
						hi = h[v];
	...
	pair<flowUnit, costUnit> Cost(int V, int s = -1, int t = -1) {
